...
router.get('/', async (ctx:Koa.Context) => { 
    ...
    router.get('/:movie_id', async (ctx:Koa.Context) => { 
        ...
        if (!movie) { ctx.throw(HttpStatus.NOT_FOUND); } 
        // Respond with our movie data. 
        ctx.body = { data: { movie }, }; }); 
        ...
        router.post('/', async (ctx:Koa.Context) => { 
         // Get the movie repository from TypeORM. 
           ...
           const movie:movieEntity = ctx.request.body;
            // Persist it to the database. 
             ...
             router.delete('/:movie_id', async (ctx:Koa.Context) => {
                  ...
                  // This will be handled upstream by our custom error middleware. 
                  if (!movie) { ctx.throw(HttpStatus.NOT_FOUND); } 
                  // Delete our movie. await movieRepo.delete(movie); 
                  ...
                  router.patch('/:movie_id', async (ctx:Koa.Context) => { 
                      ...
                      // Save the new data. movieRepo.save(updatedMovie); 
                      // Respond with our movie data.
                      // Response with the updated content. 
                      ctx.body = { data: { movie: updatedMovie }, }; }); 
